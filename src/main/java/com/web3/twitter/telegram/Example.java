...
public final class Example {
	...
	public static void main(String[] args) throws Exception {
		...
		try (SimpleTelegramClientFactory clientFactory = new SimpleTelegramClientFactory()) {
			...
			try (ExampleApp app = new ExampleApp(clientBuilder, authenticationData, adminId)) {
				// Get me
				TdApi.User me = app.getClient().getMeAsync().get(1, TimeUnit.MINUTES);

				// Create the "saved messages" chat
				Object savedMessagesChat = app.getClient().send(new CreatePrivateChat(me.id, true)).get(1, TimeUnit.MINUTES);

				// Send a test message
				SendMessage req = new SendMessage();
				//req.chatId = savedMessagesChat.id;
				req.chatId = Long.parseLong(savedMessagesChat.toString());
	...
	public static class ExampleApp implements AutoCloseable {
		...
		private void onUpdateNewMessage(TdApi.UpdateNewMessage update) {
			...
			// Get the chat title
			client.send(new GetChat(chatId))
